      ...
      <tfoot>
        <tr>
          <th colspan="<?php echo count($this->configuration->getValue('list.display')) + ($this->configuration->getValue('list.object_actions') ? 1 : 0) + ($this->configuration->getValue('list.batch_actions') ? 1 : 0) ?>">
            [?php // commented by vit // if ($pager->haveToPaginate()): ?]
              [?php include_partial('<?php echo $this->getModuleName() ?>/pagination', array('pager' => $pager)) ?]
            [?php // commented by vit // endif; ?]

            [?php echo format_number_choice('[0] нет записей|[1] 1 запись|(1,+Inf] всего записей: %1% ', array('%1%' => $pager->getNbResults()), $pager->getNbResults(), 'sf_admin') ?]
            [?php // if ($pager->haveToPaginate()): ?]
              [?php echo __('(страница %%page%%/%%nb_pages%%)', array('%%page%%' => $pager->getPage(), '%%nb_pages%%' => $pager->getLastPage()), 'sf_admin') ?]
            [?php // endif; ?]
          </th>
        </tr>
      </tfoot>
